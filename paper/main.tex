...
which rearranges to \eqref{eq:identity}. For the inequality, $\cos$ is convex on
$[0,\pi/2]$, so Jensen gives $\frac1d\sum_j\cos\theta_j\ge\cos(\frac1d\sum_j\theta_j)$;
since $\arccos$ is decreasing, $\Thetaglob\le\frac1d\sum_j\theta_j=:\bar\theta$.
Squaring and applying QM--AM $(\frac1d\sum\theta_j)^2\le\frac1d\sum\theta_j^2$ yields
$\Thetaglob^2\le\bar\theta^2\le\frac1d\sum_j\theta_j^2$.
\end{proof}
...
seeds), and for each of the $72$ target weight matrices per run measure the per-column
...
$\hat W=W_0+\Delta W$. Pooling gives $72\times36=2592$ measured layers. The directional
...
raw-vs-unit flattening offset, not a violation.
...
(Fig.~\ref{fig:thm}, right). The exact value sits at the lower end of the bracket
